...
class Lap_kas_anggota extends OPPController {
	...
	public function getdatabydate(){
		$data_detail_anggota = $this->lap_kas_anggota_m->get_detail_data_cetak();
		echo json_encode($data_detail_anggota);
	}

	public function getsaldoakhir(){
		$saldo  =$this->lap_kas_anggota_m->get_saldo_akhir();
	...
	public function index() {
		...
		$this->data["data_detail_anggota"] = $this->lap_kas_anggota_m->get_detail_data(); 
	...
	function cetak_laporan() {
     ...
     $html .= '
         ...
         if (isset($_REQUEST['tgl_dari']) && $_REQUEST['tgl_dari'] != '') {
         	$html .= $pdf->nsi_box($text = '<span class="txt_judul">Laporan Data Kas Simpanan Anggota</span>', $width = '100%', $spacing = '0', $padding = '1', $border = '0', $align = 'center');
         	...
         	$html .= $pdf->nsi_box($text = '<span class="txt_judul">Laporan Data Kas Anggota</span>', $width = '100%', $spacing = '0', $padding = '1', $border = '0', $align = 'center');
         }
      ...
      $html .= '
		...
		<tr class="header_kolom">
			...
			<th class="h_tengah" style="width:24%;"> Tanggal </th>
			<th class="h_tengah" style="width:13%;"> Akun </th>
		...
		$no =1;
		$jml_simpanan = 0;
		foreach ($data_detail_anggota as $row) {
			...
			$txt_tanggal = str_replace("jam", "-", jin_date_ina($row->tgl_transaksi,"full",true));
		...
		$html .= '
		<tr>
			<td colspan="6" class="h_tengah"><strong> Saldo Akhir </strong></td>
			<td class="h_kanan"> <strong>'.number_format($simpanan_row_total).'</strong></td>
		</tr>
		</table>';
		}
